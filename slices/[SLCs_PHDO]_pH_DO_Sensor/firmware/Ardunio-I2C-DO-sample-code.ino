...
#define address 99               //default I2C ID number for EZO DO Circuit.
...
void loop() {                                                                     //the main loop.
 if (serial_event == true) {                                                      //if a command was sent to the EZO device.
    ...
    else if (computerdata[0] == 'r')
    {
      time_ = 900;             //if a command has been sent to calibrate or take a reading we wait 575ms so that the circuit has time to take the reading.
    }
    else 
    {
      time_ = 300;                                                             //if any other command has been sent we wait only 250ms.
    }
